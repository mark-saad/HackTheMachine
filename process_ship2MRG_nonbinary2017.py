...
data = read_csv("D:\HackTheMachine\hackthemachine-data\gas\classB_ship2_allMRG_sorted_rmDUPs_non-binary_2017.csv")
#print data.head()
...
for key in df_dict.keys():
    ...
    #print df.head()
    ax = df.plot(kind='line',logy=True, title='classB_ship2_meanfilled_nonbinary2017' + key).legend(loc='center left', bbox_to_anchor=(1, 0.5))
    fig = ax.get_figure()
    fig.savefig('classB_ship2_meanfilled_nonbinary2017_' + key + '.png')
    df.to_csv('classB_ship2_meanfilled_nonbinary2017_' + key + '.csv', sep=',')