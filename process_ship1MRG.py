...
data = read_csv("D:\HackTheMachine\hackthemachine-data\gas\classA_ship1_allMRG_sorted_rmFlatCols_rmDUPs.csv")
#print data.head()
...
for key in df_dict.keys():
    ...
    #print df.head()
    ax = df.plot(kind='line',logy=True, title='classA_ship1_meanfilled_' + key).legend(loc='center left', bbox_to_anchor=(1, 0.5))
    fig = ax.get_figure()
    fig.savefig('classA_ship1_meanfilled_' + key + '.png')
    df.to_csv('classA_ship1_meanfilled_' + key + '.csv', sep=',')